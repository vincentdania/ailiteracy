...
export default async function LessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="mx-auto max-w-3xl">
    ...
    {caseStudy && <aside className="mt-8 rounded-2xl bg-[#00261d] p-6 text-white sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><p className="eyebrow text-[#fec24a]">African case study · {caseStudy.sector}</p><span className="flex items-center gap-1 text-xs font-bold text-white/60"><MapPin size={14} />{caseStudy.region}</span></div><h2 className="display mt-3 text-4xl">{caseStudy.name}</h2><p className="mt-4 leading-7 text-white/75">{caseStudy.summary}</p><div className="mt-5 rounded-xl bg-white/8 p-5"><p className="flex items-center gap-2 font-bold text-[#fec24a]"><Lightbulb size={18} />What to notice</p><p className="mt-2 leading-7 text-white/75">{caseStudy.lesson}</p></div><a href={caseStudy.sourceUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-white underline decoration-white/40 underline-offset-4">Read the original source: {caseStudy.sourceLabel} <ExternalLink size={15} /></a></aside>}
